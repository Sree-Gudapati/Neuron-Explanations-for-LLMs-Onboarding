...
# ==========================================
# [START] M1/APPLE SILICON SUPPORT
# ==========================================
# 1. Detect Device
if torch.backends.mps.is_available():
    device = torch.device("mps")
    print(f"✅ analyze.py: Using Apple MPS (M1/M2/M3) acceleration")
elif torch.cuda.is_available():
    device = torch.device("cuda")
    print(f"✅ analyze.py: Using NVIDIA CUDA")
else:
    device = torch.device("cpu")
    print(f"⚠️ analyze.py: Using CPU")

# 2. Fix MacOS Multiprocessing Crash
try:
...
def extract_features(
    ...
    for src, src_feats, src_multifeats, src_lengths, idx in tqdm(loader):
        ...
        # [MODIFIED] M1 Support: Use global device instead of checking settings.CUDA
        src = src.to(device)
...
def search_feats(acts, states, feats, weights, dataset):
    ...
    # --- START OF M1 FIX ---
    # Replaced broken multiprocessing pool with a standard sequential loop
    # This runs on a single core to avoid memory errors on macOS
    
    for arg in tqdm(mp_args, desc="Units"):
        res = ioufunc(arg)  # Call the function directly
...
def to_sentence(toks, feats, dataset, tok_feats_vocab=None):
    ...
    if tok_feats_vocab is None:
        ...
        for oth, oth_type in oth_names:
            # FIX: Used to be 'oth_name', fixed to 'oth'
...
def main():
    ...
    model, dataset = data.snli.load_for_analysis(
        ...
        # We handle device manually below to avoid CUDA errors in loading if settings.CUDA is True
        cuda=False, 
    )
    
    # [MODIFIED] Explicitly move model to M1/CUDA device
